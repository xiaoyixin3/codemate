...
public class ForumExceptionHandler implements HandlerExceptionResolver {
    ...
    public ModelAndView resolveException(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) {
        ...
        if (restResponse(request, response, handler)) {
            // 表示返回json数据格式的异常提示信息
     ...
     * @return
     */
    private boolean restResponse(HttpServletRequest request, HttpServletResponse response, Object handler) {
        if (handler instanceof HandlerMethod
                && ((HandlerMethod) handler).getBeanType().isAnnotationPresent(RestController.class)) {
            return true;
        }
