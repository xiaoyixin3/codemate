...
public class UserDao extends ServiceImpl<UserInfoMapper, UserInfoDO> {
    ...
    /**
     * Upgrade a legacy password only when it has not changed since it was read.
     */
    public boolean upgradePasswordIfMatch(Long userId, String oldPassword, String newPassword) {
        return userMapper.update(null, Wrappers.lambdaUpdate(UserDO.class)
                .set(UserDO::getPassword, newPassword)
                .eq(UserDO::getId, userId)
                .eq(UserDO::getPassword, oldPassword)) == 1;
    }
}
