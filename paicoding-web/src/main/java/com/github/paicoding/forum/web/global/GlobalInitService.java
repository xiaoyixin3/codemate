...
public class GlobalInitService {
    ...
    public void initLoginUser(ReqInfoContext.ReqInfo reqInfo) {
        ...
        // API clients authenticate with the standard Authorization header. When it is present,
        // do not fall back to a browser cookie: an explicitly supplied invalid token must not
        // silently authenticate as the cookie owner.
        String authorization = request.getHeader(AUTHORIZATION_HEADER);
        if (StringUtils.startsWithIgnoreCase(authorization, BEARER_PREFIX)) {
            initLoginUser(authorization.substring(BEARER_PREFIX.length()).trim(), reqInfo);
            return;
        }
